# this one does not work as the motor does not move under playing mode
...
polling_active  = True
motor_moving    = False  # prevents overlapping move commands
...
def poll():
    while polling_active:
        ...
        time.sleep(0.15)  # ~7 Hz — smooth enough, reduces IMU noise jitter
...
def on_set_sensor(data):
    ...
    print(f"[~] Sensor type set to: {current_sensor}")
...
def on_move_motor(data):
    ...
    # Skip if a move is already in progress to avoid queuing up commands
    if motor_moving:
        return
    ...
    def _move():
        ...
        try:
            ...
            # Always take the shortest path (handles 350° → 10° correctly)
            if diff > 180:
                diff -= 360
            if diff < -180:
                diff += 360
            ...
            if abs(diff) > 5:
                safe(lambda: motor.motor_run_for_degrees(int(diff)))
        finally:
            motor_moving = False
